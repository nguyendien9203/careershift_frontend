...
const InterviewSchedule: React.FC = () => {
  ...
  // State for stage creation and evaluation
  const [newStageType, setNewStageType] = useState("TECHNICAL_INTERVIEW");
  const [newStageInterviewerIds, setNewStageInterviewerIds] = useState<
    string[]
  >([]);
  const [currentRound, setCurrentRound] = useState<number | null>(null);
    ...
    const fetchInitialData = async () => {
      try {
        ...
        if (recruitmentRes.data.success)
          setRecruitmentOptions(recruitmentRes.data.data);
        setAvailableInterviewers(usersRes.data);
      } catch (err) {
        setError("Lỗi khi tải dữ liệu ban đầu");
      } finally {
  ...
  const fetchInterviewDetails = async (id: string) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:9999/api/interviews/${id}`
      );
      if (response.data.success) {
        setSelectedInterview(response.data.data);
        await fetchAdditionalDetails(response.data.data);
  ...
  const fetchAdditionalDetails = async (interview: Interview) => {
    try {
      ...
      const [candidateRes, jobRes] = await Promise.all([
        axios.get(
          `http://localhost:9999/api/candidates/${recruitment.candidateId}`
        ),
        axios.get(`http://localhost:9999/api/jobs/${recruitment.jobId}`),
      ]);
      ...
      const interviewerIds = Array.from(
        new Set(interview.stages.flatMap((s) => s.interviewerIds))
      );
      if (interviewerIds.length) {
        const interviewerResponses = await Promise.all(
          interviewerIds.map((id) =>
  ...
  const handleAddInterview = async () => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:9999/api/createInterviews",
        {
          recruitmentId: addRecruitmentId,
          date: new Date(addDate).toISOString(),
          time: addTime,
          mode: addMode,
          ...(addMode === "ONLINE"
            ? { google_meet_link: addGoogleMeetLink }
            : { address: addAddress }),
        }
      );
      if (response.data.success) {
  ...
  const handleCreateStage = async () => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:9999/api/createInterviewStage",
        {
          interviewId: selectedInterview._id,
          round: selectedInterview.stages.length + 1,
          type: newStageType,
  ...
  const handleConductStage = async () => {
    ...
    try {
      const response = await axios.put(
        "http://localhost:9999/api/interviews/update-stage",
        {
          interviewId: selectedInterview._id,
          round: currentRound,
          interviewerId: selectedInterviewerId,
  ...
  const completedCount = interviews.filter(
    (i) => i.finalStatus === "COMPLETED"
  ).length;
  const canceledCount = interviews.filter(
    (i) => i.finalStatus === "CANCELLED"
  ).length;
            ...
            <tbody>
              {filteredInterviews.map((interview) => (
                <tr key={interview._id}>
                  ...
                  <td style={styles.td}>
                    {interview.finalStatus === "IN_PROGRESS"
                      ? "Đang tiến hành"
                      : interview.finalStatus === "COMPLETED"
                      ? "Hoàn thành"
                      : "Đã hủy"}
                  </td>
                  <td style={styles.td}>
                    ...
                    <button
                      ...
                      onClick={() =>
                        handleUpdateField(
                          `http://localhost:9999/api/cancel/${interview._id}`,
                          {}
                        )
                      }
                    >
        ...
        <div style={styles.modalOverlay}>
          <div style={styles.addModalContent}>
            ...
            >
              ...
              <button
                style={styles.modalCloseButton}
                onClick={() => setAddInterviewModal(false)}
              >
                Đóng
              </button>
            </div>
            ...
            <div style={styles.modalButtonGroup}>
              ...
              <button
                ...
                onClick={() =>
                  axios
                    .post(
                      "http://localhost:9999/api/send-interview-invitation",
                      {
                        interviewId: selectedInterview._id,
                      }
                    )
                    .then(
                      (res) =>
                        res.data.success && alert("Gửi email thành công!")
                    )
                }
              >
                Gửi Email
            ...
            {detailsLoading ? (
              ...
              <>
                ...
                <div style={styles.modalSection}>
                  <div style={styles.modalLabel}>Trạng thái cuối cùng</div>
                  <div style={styles.modalValue}>
                    {selectedInterview.finalStatus === "IN_PROGRESS"
                      ? "Đang tiến hành"
                      : selectedInterview.finalStatus === "COMPLETED"
                      ? "Hoàn thành"
                      : "Đã hủy"}
                  </div>
                  <button
                ...
                <div>
                  ...
                  {selectedInterview.stages.map((stage) => (
                    <div key={stage.round} style={styles.stageSection}>
                      <div style={styles.stageHeader}>
                        Vòng {stage.round} -{" "}
                        {stage.type === "HR_SCREENING"
                          ? "Sàng lọc HR"
                          : stage.type === "TECHNICAL_INTERVIEW"
                          ? "Phỏng vấn Kỹ thuật"
                          : "Phỏng vấn Cuối"}
                      </div>
                      <div style={styles.modalSection}>
                      ...
                      <div style={styles.modalSection}>
                        ...
                        <div style={styles.modalValue}>
                          {stage.interviewerIds
                            .map((id) => interviewers[id]?.email || id)
                            .join(", ")}
                        </div>
                      </div>
                      <div style={styles.modalSection}>
                          ...
                          {stage.evaluations.length
                            ? stage.evaluations.map((e, i) => (
                                <div key={i}>
                                  <div>
                                    Người đánh giá:{" "}
                                    {interviewers[e.interviewerId]?.email ||
                                      e.interviewerId}
                                  </div>
                                  <div>
                                    Điểm số:{" "}
        ...
        <div style={styles.modalOverlay}>
          <div style={styles.addStageModalContent}>
            ...
            >
              ...
              <button
                style={styles.modalCloseButton}
                onClick={() => setAddStageModal(false)}
              >
                Đóng
              </button>
            </div>
        ...
        <div style={styles.modalOverlay}>
          <div style={styles.conductModalContent}>
            <h2 style={styles.modalHeader}>
              Conduct Stage - Round {currentRound}
            </h2>
            <div style={styles.modalSection}>
              <div style={styles.modalLabel}>Evaluator</div>
              <select
            ...
            <div style={styles.modalSection}>
                    ...
                    <tr key={index}>
                      ...
                      <td style={styles.td}>
                        <button
                          ...
                          onClick={() =>
                            setScoreEntries(
                              scoreEntries.filter((_, i) => i !== index)
                            )
                          }
                          disabled={scoreEntries.length === 1}
                        >
              ...
              <button
                style={styles.button}
                onClick={() =>
                  setScoreEntries([...scoreEntries, { key: "", value: "" }])
                }
              >
                Add Criterion
              </button>
            ...
            >
              ...
              <button
                style={styles.modalCloseButton}
                onClick={() => setConductStageModal(false)}
              >
                Close
              </button>
            </div>
          ...
          <div style={styles.editModalContent}>
            ...
            >
              <button
                ...
                onClick={() =>
                  handleUpdateField(
                    "http://localhost:9999/api/updateInterviewDateTime",
                    {
                      interviewId: selectedInterview!._id,
                      date: editDate,
                      time: editTime,
          ...
          <div style={styles.editModalContent}>
            ...
            >
              <button
                ...
                onClick={() =>
                  handleUpdateField(
                    `http://localhost:9999/api/update-mode/${
                      selectedInterview!._id
                    }`,
                    {
                      mode: editMode,
                      ...(editMode === "ONLINE"
                        ? { google_meet_link: editGoogleMeetLink }
                        : { address: editAddress }),
                    }
                  ).then(() => setEditModeModal(false))
                }
          ...
          <div style={styles.editModalContent}>
            ...
            >
              <button
                ...
                onClick={() =>
                  handleUpdateField(
                    "http://localhost:9999/api/updateFinalStatus",
                    {
                      interviewId: selectedInterview!._id,
                      finalStatus: editFinalStatus,
                    }
          ...
          <div style={styles.editModalContent}>
            ...
            >
              <button
                ...
                onClick={() =>
                  handleUpdateField(
                    "http://localhost:9999/api/update-pass-fail",
                    {
                      interviewId: selectedInterview!._id,
                      round: editRound,
                      status: editPassFailStatus,
