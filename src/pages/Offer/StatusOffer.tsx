        ...
        const fetchOffers = async () => {
            try {
                ...
                const url = selectedStatus === 'ALL'
                    ? 'http://localhost:5000/api/offers/listStatus'
                    : `http://localhost:5000/api/offers/listStatus?status=${selectedStatus}`;
                ...
                const recruitmentPromises = rawOffers.map(async (offer) => {
                    ...
                    try {
                        const recruitmentResponse = await axios.get<RecruitmentResponse>(
                            `http://localhost:5000/api/recruitments/${offer.recruitmentId}`
                        );
                        const recruitment = recruitmentResponse.data.recruitment;
                        return {
                            ...offer,
                            candidateName: recruitment?.candidateId?.name || 'Unknown Candidate',
                ...
                {isModalOpen && (
                    <div className="modal-overlay">
                        <div className="modal-content">
                            ...
                            <form
                                onSubmit={(e) => {
                                    console.log('Form submitted with selectedOfferId:', selectedOfferId);
                                    handleUpdateSubmit(e);
                                }}
                            >                                <div className="form-group">
                                    <label htmlFor="negotiatedSalary">Lương thương lượng (VND):</label>
                                    <input
                                        type="number"
                                        id="negotiatedSalary"
                                        value={negotiatedSalary}
                                        onChange={(e) => setNegotiatedSalary(Number(e.target.value))}
                                        required
