...
const Input = forwardRef<HTMLInputElement, InputProps>(({
  ...
  return (
    <div
      className={`${layout === "horizontal" ? "flex items-start" : ""} w-full`}
    >
      {label && (
        <label
      ...
      <div className="relative w-full">
        ...
        {error && layout === "horizontal" && <p className="text-red-500 text-xs mt-1">{error}</p>}
      </div>
      {error && layout === "vertical" && <p className="text-red-500 text-xs mt-1">{error}</p>}
    </div>
  );
});
